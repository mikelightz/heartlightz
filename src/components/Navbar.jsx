...
const Navbar = ({ setCurrentPage, currentPage }) => {
  ...
  const navLinks = [
    ...
    { name: 'Shop', action: () => { setCurrentPage('shop'); window.scrollTo({ top: 0, behavior: 'smooth' }); } },
          ...
          {navLinks.map((link) => {
            ...
            return (
              <a
                ...
                className={`block w-full py-3 px-4 bg-black border-[4px] border-[#94150D] ${isActive ? '[border-style:inset] bg-[#94150D] text-white' : '[border-style:outset] hover:[border-style:inset] hover:bg-[#94150D] text-[#FCDFB9] hover:text-white'} font-bold tracking-widest uppercase text-center transition-none shadow-[2px_2px_0px_#000]`}
              >
                {link.name}
              </a>
            );
